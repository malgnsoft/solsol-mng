...
export type StatusPatch = Partial<Pick<StatusRow, 'design' | 'publish' | 'review' | 'spike' | 'dev' | 'test' | 'mockupUrl' | 'devUrl'>>
...
function applyPatch(prev: StatusRow | undefined, id: string, patch: StatusPatch, defaults?: Partial<StatusRow>): StatusRow {
  ...
  const base: StatusRow = prev ?? { screenId: id, design: true, publish: false, review: false, spike: false, dev: false, test: false, mockupUrl: '', devUrl: '', updatedAt: null, ...defaults }
  ...
  return {
    ...
    upsert: async (id, patch, defaults) => {
      ...
      await db.insert(screenStatus).values(row)
        .onConflictDoUpdate({ target: screenStatus.screenId, set: { design: row.design, publish: row.publish, review: row.review, spike: row.spike, dev: row.dev, test: row.test, mockupUrl: row.mockupUrl, devUrl: row.devUrl, updatedAt: row.updatedAt } })
      return row
    },
  }
}
