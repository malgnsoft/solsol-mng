...
| PRV-cttee-r02-D01 | committee-r02 | solsol-brand-api `GET /admin/users`·`/:id` | 상 | 플랫폼 관리자(타인)에게 셀러 loginId·email·name·phone 전부 무마스킹 노출(mask 함수 dead code) — C-6 타인조회 마스킹 위반 | **closed** | brand-fullstack(2026-07-02) | working tree에 마스킹 기구현 확인(`users.ts:104-107,204-207` maskLoginId/Email/Name/Phone + isSelfRequest 서버강제, password_hash 응답제외). **privacy(노개보)+security(배보검) 공동서명 종결** — 재구현 아님. 신규 중=maskPhone fail-open·loginLogs ipAddr 노출(별건) |
| SEC-VC-01 | brand-fullstack (2026-07-02) | solsol-brand-api `POST /api/auth/email-code/issue`·`/verify` (A5/A6) | 상 | issueToken이 `codeHash=sha256(email:code)`를 평문 JSON+HMAC 서명(비암호화) 본문으로 클라 반환 → 임의 victim 이메일 issue 후 토큰 디코드→codeHash 추출→6자리 오프라인 완전탐색→verifyToken 위조→이메일 소유증빙 우회. 레이트리밋 무력(오프라인) | open | — | 해제조건: backend-db 코드 원장 테이블(email+purpose 키·시도카운터·1회성 소진) 도입 + codeHash 클라 반환 폐기(서버측 코드 대조). verify-code 운영배포 차단(실연동 blocker0 강제). 소유=backend-db/dba+api-developer, CF 레이트리밋은 보조 |
