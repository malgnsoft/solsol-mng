...
> **모달/팝업 `_pu` 화면ID 추적(2026-06-25, DJ 지시)**: 본문에 `-P01/-P02` 등으로 정의된 고유 콘텐츠 모달/팝업은 `00_화면목록.md §3.1.2`에서 `부모화면ID_pu##`로 채번됐다(기존 `-P##` → `_pu##` 1:1, 외부·범용 제외분 재순번 1건=결제 0201-001). FR 모달/팝업 14건. 매핑 SoT는 §3.1.2 하위표(재증류 없이 ID 추적키만 부착).
...
### S-FR01-0110-001 / -002 / -003 / -005 자유게시판 (목록·상세·글쓰기·수정)
<!-- 채번 정정(02_정합성재검증 N-1): 실화면 4개(목록 -001 / 상세 -002 / 글쓰기 -003 / 수정 -005). -004는 미사용 공번이라 헤더에서 제거. SCREEN ID 필드와 일치. 화면 수·정책 불변. -->
...
3. **1 [받을 수 있는 쿠폰]** — 아직 발급받지 않은 쿠폰 노출. 기본 정렬 = 쿠폰 최신 등록순.
   ...
   3.3. **1-3 할인정보** — 정액 `-{n}원`(오픈 시점). `[C-4 확정: 오픈 시점 정액 only — 정률(%) 표기 숨김(hide), 정률 P1 이관 / 원 Figma 카드 "정률 {n}% 할인" 표기 supersede]`
...
| P-41 | S-FR01-0301-105 | 쿠폰=받을수있는(미발급, [쿠폰받기]→토스트→받기완료 비활성·사용가능 목록 생성) + 사용가능(발급분, 표시 전용) 2섹션. 카드=사용기간(`YYYY.MM.DD HH:MM 까지`)+쿠폰명+할인(정액 -{n}원, 오픈 시점)+최소주문금액. 최신 등록/발급순. 결제 쿠폰 선택(P-30) 연동. **[C-4 확정: 오픈 시점 정액 only — 정률(%) 표기 숨김(hide), 정률 P1 이관 / 원 Figma "정률 {n}%" 표기 supersede]** |
