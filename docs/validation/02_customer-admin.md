...
2. **역할(Role) 배지 4종 — 관리자/강사/학습자/서브강사** — p016 배지 컴포넌트: `관리자`(핑크)·`강사`(보라)·`학습자`(하늘)·`서브강사`(배지없음 회색). 어드민 접속 주체는 관리자·강사(서브강사 포함) 추정, 학습자 배지는 사용자 목록에서 대상 표기용. 강사/서브강사 계정의 LNB 메뉴·기능 노출 범위는 **미확정**, 각 화면 권한 캡처 발견 시 명시 `[→ 임기획/강테크]`.
...
- 권한: 강사 계정은 일부 메뉴 비노출 추정(P-AD-02).
...
**DESCRIPTION**
1. 진입: 최고관리자 계정으로 로그인 시 최초 노출되는 메인 페이지(P-AD-04). 강사 계정 진입 동작은 미확정 `[미확정-강사권한]`.
...
3. **[비밀번호 변경 완료]**: 입력 데이터 최종 검증 후 DB 반영 요청.
   ...
   3.2. 실패 → **오류 모달**(p031, 신규 26-04-25: "오류가 발생했습니다 / 비밀번호는 영문, 숫자, 특수문자를 포함하여 8자 이상이어야 합니다" + [확인]).
4. [← 로그인 화면으로 돌아가기].
...
2. **입력 폼**(모두 필수 `*`):
   ...
   2.2. 닉네임: placeholder "닉네임을 입력하세요", 화면 안내 "2~20자, 다른 사용자에게 표시되는 이름입니다"(⚠ Description은 **최소 2자·최대 10자**로 명시 — 안내문/규칙 불일치, P-AD-10 미확정). 실시간 유효성+중복 체크: 미입력="닉네임을 입력해주세요" / 특수문자="닉네임은 한글, 영문, 숫자만 사용 가능합니다" / 미달="닉네임은 최소 2자 이상 입력해주세요" / 초과="닉네임은 10자 이내로 입력해주세요" / 중복="이미 사용 중인 닉네임입니다" / **금칙어**(관리자·어드민·admin·고객센터·CS·공지사항·도우미·욕설/비속어/성적 언어, 공백/특수문자 제거 후 대조)="사용할 수 없는 단어가 포함되어 있습니다" / 공백="닉네임에 공백을 제거해 주세요".
...
2. **계정 정보**:
   ...
   2.4. 닉네임(필수): [n/15] 카운트(⚠ 회원가입 규칙은 최대 10자 — 화면 간 불일치, P-AD-10).
...
5. **일괄 액션 플로팅바**(C-8): 체크 1개↑ 선택 시. **[활성]/[중지]/[권한 설정]/[탈퇴]**.
   ...
   5.2. **[권한 설정]**: **권한 설정 모달**(p123, LPU, P-AD-19) — ① 권한 설정 단계: 대상 강사 칩 + [강사 추가] + **메뉴 권한 체크트리**(알림보기 / 상품(일반강의·화상강의·라이브강의·디지털상품·패키지상품·멤버십·프리미엄커뮤니티) / 콘텐츠 / 판매 등, 상위 체크 시 하위 연동) + [설정 완료](안내문 "설정 완료 버튼을 누르면 이전 권한은 무시되고 현재 모달 설정값으로 일괄 갱신"). ② 강사 추가 단계: 강사명/이메일 검색 + 강사 목록([추가]) + 선택된 강사 {n} + [저장].
...
7. 상태:
   ...
   7.2. 로딩/에러: `[추정]`.
8. 권한: 강사 계정은 본인 개설 상품만 노출 추정 `[미확정-강사권한]`(P-AD-02).
...
6. 권한: 강사 계정 상품 생성 권한 범위 `[미확정-강사권한]`(P-AD-02).
...
6. **카카오 채널 연결 모달**(p505, LPU): "발송내역>목록>알림톡 연결 모달". 카카오 비즈니스 채널 계정을 개설 → 채널 아이디 연결.
...
5. **④ 미리보기(캔버스) 영역**: 템플릿 선택 시 섹션 조합 노출 → 직접 수정(텍스트/이미지/버튼). 기존 구성 변경 시(처음 한 번만) 초기화 경고.
   5.1. **섹션 편집 패널**(섹션 선택 시 좌측 패널 전환, P-AD-102): 섹션 유형별 입력 폼 + 우측 라이브 미리보기 + [섹션 삭제]. 대표 유형:
      ...
      d. **게시판 섹션**(p658/p661): 게시글 선택 모달(검색·게시판 유형 필터 전체/공지/자유/FAQ/프리미엄·**다중 선택**·게시판 유형+게시판명+게시글 제목·[추가]). 검색어+유형 AND.
      e. **후기 섹션**(p659/p660): 후기 선택 모달(검색·**다중 선택**·상품유형+상품명+후기 제목·[추가]·default 전체 후기글 최신순).
      f. **로케이션 섹션**(p645/p650): 구글맵 설정([주소 검색] 모달→우편번호/도로명/지번·구글맵 호출)·contact(대표번호/팩스=사이트>푸터 값)·배경색.
      g. **설문 섹션**(p648/p653): 설문폼 선택(마케팅 지원 설문폼·발행 상태만)·응답자/응답 기간 read·배경색.
...
| P-AD-02 | LNB 메뉴 권한 차등 | 강사/서브강사 계정은 LNB 일부 메뉴만 노출(권한 설정 기반) `[미확정-범위]` | C-1, p002 |
...
| P-AD-09 | 어드민 회원가입 검증 | 이름(한글, 공백X, 20자)·닉네임(2~10자, 금칙어)·이메일 인증(코드 10분)·비번(8~16자 3종 조합)·약관 동의 | S-AD01-0303-001 |
| P-AD-10 | 닉네임 길이 규칙 불일치 | 화면 안내 "2~20자" vs Description "2~10자" — 확정 필요 `[미확정]` | S-AD01-0303-001, S-AD01-9003-001 |
...
| P-AD-19 | 강사/관리자 권한 설정 | 권한 설정 모달=메뉴 체크트리(상위→하위 연동), [설정 완료] 시 기존 권한 무시·일괄 갱신. 강사/관리자 추가 단계로 다중 적용. **추가 대상 목록은 탈퇴/중지 회원 비노출**(p154). 관리자 모달도 강사와 동형 | S-AD01-0102/0103 (p123/p153/p154) |
...
> **에스컬레이션 후보**: P-AD-10(닉네임 길이 불일치), P-AD-02(강사 메뉴 권한 범위 미확정), P-AD-11(알림 기획 미진행). §0과 연동.
...
> - 미확정: 강사 메뉴 권한 범위(P-AD-02)·콘텐츠 라이브러리 작은 Description 세부(AI 차감 단가)·AI 크레딧 실제 차감 정책(개발 시 확정)·쿠폰 정률 추후 확장(DB 스키마)·게시판 게시글 쓰기/상세(커뮤니티 동형 추정)·자유게시판 게시글 동작(추정)·환불 디지털 상품 예외(추정).
