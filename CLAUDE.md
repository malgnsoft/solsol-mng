...
- **게이트**: 구현/셀프검증 → qa·security·privacy(+dba) 게이트 → 팀장 컨펌(**❌'상' 0건**) → 배포. 심각도 **상/중/하** 단일. **blocker는 [docs/dev-validation/_ledger.md] 원장**에 등재(우회·강등은 승인자 기재).
- **기록**: 결함표는 `validation/` **밖** `docs/dev-validation/<영역>-r<NN>.md`(표준 템플릿), 배포 시 `docs/history/` 요약. 추적키 1:1 매핑(누락 0).
